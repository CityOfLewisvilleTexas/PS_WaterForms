...
var app = new Vue({
  el: '#app',
  data: {
    record: '', // testing: '3b565326a6ee438da9354e5cdb9ad30f'
    data: [],
    previousYear: [],
    yearParam: '',
    loading: false,
  ...
  methods: {
    fetchData: function() {
      ...
      if (self.record !== undefined) {
        ...
        $.post('https://query.cityoflewisville.com/v2/', {
          webservice : 'Public Services/Water/viewWaterElectrical',
          record : self.record,
        }, function(data) {
          self.data = data[0].slice(1);
          self.previousYear = data[0][0];
          console.log(data[0]);
          self.loading = false;
        });
      } else {
    ...
    getDifference: function(value, index) {
      var previous = (index > 0) ? parseFloat(this.data[index - 1][value]) : parseFloat(this.previousYear[value]);
      ...
      if (isNaN(previous) || isNaN(current)) {
        ...
        if (current >= previous) {
          ...
          return ((current + 100000) - previous).toFixed(1);
        }
