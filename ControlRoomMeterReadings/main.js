...
var app = new Vue({
  ...
  methods: {
    ...
    getTotal: function(value, index) {
      var self = this;
      var previous = parseInt((index > 0) ? self.data[index - 1][value] : self.previousMonth[value]);
      var current = parseInt(self.data[index][value]);
      var total;
      if (current !== null) {
        if (value === "Sludge") { // Sludge meter totals have different calculations
			previous = parseFloat((index > 0) ? self.data[index - 1][value] : self.previousMonth[value]);
			current = parseFloat(self.data[index][value]);
    ...
    combinedTotal: function(value1, value2, index) {
      ...
      if (nsTotal) {
        return (parseFloat(plantTotal) + parseFloat(nsTotal)).toFixed(3);
      } else {
        return 0;
      }
    },
    columnCalc: function(value, value2) {
      ...
      self.data.forEach(function(date, index) {
        ...
        if (isNaN(tempVal)) {
          ...
          if (tempVal < min) { min = parseFloat(tempVal); }
          if (tempVal > max) { max = parseFloat(tempVal); }
        }
      });
      avg = (total / self.data.length);
      return {total: total.toFixed(3), avg: avg.toFixed(3), min: min.toFixed(3), max: max.toFixed(3)};
    },
